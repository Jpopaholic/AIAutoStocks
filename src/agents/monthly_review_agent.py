...
def run_monthly_review(year: int, month: int, is_paper: bool = False, call_gemini_fn: Optional[Any] = None) -> Dict[str, Any]:
    ...
    for stock_code, stock_info in per_stock_data.items():
        map_prompt = (
            ...
            f"- 實際成交訂單筆數: {len(stock_info.get('orders', []))}\n"
            ...
            f"- 成交單詳細紀錄: {json.dumps(stock_info.get('orders', []), ensure_ascii=False)}\n\n"
            f"請評估該股：買入 timing 準確度、進場後 Upside/Drawdown 表現，以及分析師給分是否有偏高/偏低失真問題。"
        )
        try:
